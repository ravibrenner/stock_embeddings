...
The data we will use is [DJIA 30 Stock Time Series](https://www.kaggle.com/datasets/szrlee/stock-time-series-20050101-to-20171231) from Kaggle, which contains daily stock prices for the 30 companies in the Dow Jones Industrial Average (DJIA) from January 1, 2006 to December 31, 2017. 
...
I would say the log transformation helps with volume, but doesn't make much of a difference for the open/high/low/close features.

```{python}
df["LogVolume"] = np.log1p(df["Volume"])
```

That said, I do want to convert the open/high/low/close features to *returns* rather than prices. That is, I want the percentage change relative to the previous day's closing price. This puts them on a consistent -1 to 1 scale.

```{python}
price_features = ["Open", "High", "Low", "Close"]
df["prev_close"] = df.groupby("Name")["Close"].shift(1)
for feature in price_features:
    df[f"{feature}_ret"] = (df[feature] - df["prev_close"]) / df["prev_close"]

df.dropna(subset=["prev_close"] + [f"{f}_ret" for f in price_features], inplace=True)
```

Now we can quickly plot those for a single stock:

```{python}
ticker = "GOOGL"
df_temp = df[df["Name"] == ticker]
features_to_examine = ["Open_ret", "High_ret", "Low_ret", "Close_ret", "Volume"]
fig, axes = plt.subplots(len(features_to_examine), 2, figsize=(12, 16))
fig.suptitle('Original vs. Log-Transformed Distributions', fontsize=16)
...
scalers = {}
features_to_scale = ["Open_ret", "High_ret", "Low_ret", "Close_ret", "LogVolume"]
...
for ticker in stock_tickers:
    # We'll use a (-1, 1) range, which is often better for GRU
...
sequence_length = 20
feature_cols = ["Open_ret", "High_ret", "Low_ret", "Close_ret", "LogVolume"]
...
So the training data has dimension [64906, 20, 5], which means that there are 64,906 sequences of days, each with 20 preceding window days, and 5 features. Then there are 64,906 close prices, all corresponding to a list of 64,906 tickers. But that list of tickers is repeptitive; there are 31 tickers in total, as we would expect.
...
# Add weight_decay for L2 regularization to penalize large weights
...
close_feature_index = features_to_scale.index("Close_ret")
